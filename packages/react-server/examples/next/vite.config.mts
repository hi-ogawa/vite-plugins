  ...
  return [
        ...
        walk(ast, {
          enter(node) {
            if (
              ...
              output.update(node.start, node.end, JSON.stringify(replaced));
            }
          },
        });
    ...
    },
    {
      ...
      resolveId(source, _importer, _options) {
        // TODO: resolve font.src based on importer
      ...
      load(id, _options) {
        if (id.startsWith("\0virtual:font-extract/")) {
          ...
          if (!format) {
            throw new Error(`unknwon font format: '${font.src}'`);
          }
          const output = `
            @font-face {
              font-family: '__font_${font.id}';
...
function evaluateFont(apiName: string, code: string): FontConfig {
  const api = (arg: unknown) => arg;
  const result = (0, eval)(`(${apiName}) => ${code}`)(api);
  result.id = hashString(JSON.stringify(result)).slice(0, 8);
  return result;
}
